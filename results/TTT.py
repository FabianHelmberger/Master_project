...
if __name__ == "__main__":
    print("############################################################\n")
    for slurm_index in tqdm(range(int(16*16))):
        if slurm_index>222:
            print("############################################################")
            parameters = {
                "steps": [int(5e5)],
                "trajs": [int(1e4)],
                "dt": [1e-4],
                "sigma_abs": np.linspace(1, 5, 31)[1::2],
                "sigma_phase": np.linspace(0, np.pi*2, 31)[1::2],
                "lambda_abs": [2],
                "bins_u": [int(1e6)],
                "bins_p": [int(1e3)],
                "p_min_real": [-5],
            ...
            # Save results
            import json
            file_path = os.path.join(os.getenv("HOME"), "gitrepos", "clonscal", "results", "unmod_sim_data")
