...
def run_sim(params):
    ...
    sim.register_observable('dse_3-437_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 3.437}, langevin_history=False, thermal_time=10, auto_corr=2, maximal_lt=stop_lt)
    sim.register_observable('dse_5-468_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 5.468}, langevin_history=False, thermal_time=10, auto_corr=2, maximal_lt=stop_lt)
    ...
    with tqdm(total=stop_lt+1) as pbar:
        while cp.min(cp.asarray(sim.langevin_time)[cp.asarray(sim.alive)])<stop_lt:
            ...
            sim.step()
            sim.kill_trajs()
            # min_ada = cp.min(cp.abs(sim.ada)[cp.asarray(sim.alive)])
            # if min_ada < 1: print(f"\r min ada: {min_ada}")
    ...
    base_path = os.path.join(os.getenv("HOME"), "gitrepos", "clonscal", "bias_correction", "gaussian_sim_data", "250320")
