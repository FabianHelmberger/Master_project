...
def run_sim(params):
    ...
    # reigster observables
    sim_dse.register_observable('1_moment', obs_kernel=n_moment_kernel, const_param={"order" : 1},  langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('2_moment', obs_kernel=n_moment_kernel, const_param={"order" : 2},  langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('3_moment', obs_kernel=n_moment_kernel, const_param={"order" : 3},  langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('4_moment', obs_kernel=n_moment_kernel, const_param={"order" : 4},  langevin_history=False, thermal_time=10, auto_corr=2)
    
    sim_dse.register_observable('dse_1_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 1}, langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('dse_2_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 2}, langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('dse_3_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 3}, langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('dse_4_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 4}, langevin_history=False, thermal_time=10, auto_corr=2)
    sim_dse.register_observable('abs_drift', obs_kernel=abs_drift, langevin_history=False, thermal_time=1, auto_corr=1)
...
if __name__ == "__main__":
    ...
    for slurm_index in range(1):
        ...
        # Save results
        import json
        file_path = os.path.join(os.getenv("HOME"), "gitrepos", "clonscal", "results_1", "sim_data_gaussian", "250226")
        with open(os.path.join(file_path, f"{param_key}.json"), "w") as f:
            json.dump(results, f, indent=4)
        with open(os.path.join(file_path, f"{param_key}_hist_p.npy"), "wb") as f:
            np.save(f, hist_p)
