...
def run_sim(params):
    ...
    sim.register_observable('1_moment', obs_kernel=n_moment_kernel, const_param={"order" : 1},  langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    sim.register_observable('2_moment', obs_kernel=n_moment_kernel, const_param={"order" : 2},  langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    sim.register_observable('3_moment', obs_kernel=n_moment_kernel, const_param={"order" : 3},  langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    
    sim.register_observable('dse_1_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 1}, langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    sim.register_observable('dse_2_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 2}, langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    sim.register_observable('dse_3_moment', obs_kernel=dse_n_moment_kernel, const_param={'order': 3}, langevin_history=True, thermal_time=0, auto_corr=0.01, maximal_lt=100)
    sim.register_observable('abs_drift', obs_kernel=abs_drift, langevin_history=False, thermal_time=0, auto_corr=0.1, maximal_lt=100)
    
    # run the simf
    run_lt = 100
    min_lt = 0
    with tqdm(total=run_lt) as pbar:
        while np.min(sim.langevin_time[sim.alive])<run_lt:
            pbar.update(np.min(sim.langevin_time[sim.alive])-min_lt)
    ...
    base_path = os.path.join(os.getenv("HOME"), "gitrepos", "clonscal_bak", "thermalization", "unmod_sim_data", "250305")
...
################################################################################################
################################################################################################
parameters["steps"] = int(1e5)
parameters["trajs"] = int(1e4)
parameters["dt"] = 1e-3
...
# ################################################################################################
# ################################################################################################
# parameters["steps"] = int(5e4)
# parameters["trajs"] = int(1e3)
# parameters["dt"] = 5e-4
