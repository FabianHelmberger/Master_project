...
class Lattice(Config, Constants):
    def __init__(self, config: Config):
        ...
        self.config = config
        self.dims = np.array(self.dims, dtype=scal.IDX_TYPE)
        ...
        cumprod_dims = np.cumprod(self.sim_dims[::-1], dtype=scal.IDX_TYPE)[::-1]
        self.adims = np.zeros(self.n_dims + 1, dtype=scal.IDX_TYPE)
